...

{
  ...
  xdg.portal = {
    ...
    extraPortals = lib.mkForce [
      ...
      pkgs.xdg-desktop-portal-hyprland  # Hyprland session support
    ...
    # Some sessions export XDG_CURRENT_DESKTOP=Niri.
    config."Niri" = {
      default = [ "luminous" "gtk" ];
      "org.freedesktop.impl.portal.ScreenCast" = [ "luminous" ];
      "org.freedesktop.impl.portal.Screenshot" = [ "luminous" ];
    ...
    # Hyprland expects its own portal implementation.
    # Some environments set XDG_CURRENT_DESKTOP=Hyprland (capital H), so we
    # provide both keys.
    config.hyprland = {
